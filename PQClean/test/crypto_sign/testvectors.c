...
int main(void) {
    ...
    /* Generate 100 test vectors with random messages */
    for (i = 0; i < NTESTS; i++) {
        /* Use a fixed message length for consistency, but random content */
        size_t message_len = 32; /* 32 bytes for consistent testing */
        randombytes(mi, message_len);
        ...
        crypto_sign(sm, &smlen, mi, message_len, sk);
        crypto_sign_signature(sig, &siglen, mi, message_len, sk);
        ...
        r |= crypto_sign_verify(sig, siglen, mi, message_len, pk);
