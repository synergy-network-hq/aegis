...
# Direct WASM build script that bypasses wasm-pack
# This gives us full control over the build process and memory configuration

set -e

echo "🚀 Starting Direct WASM build process (bypassing wasm-pack)..."
...
# Build with cargo directly (bypassing wasm-pack)
print_status "Building with cargo directly (bypassing wasm-pack)..."
    ...
    if [ -d "pkg" ]; then
        ...
        if [ -f "pkg/aegis_crypto_core.wasm" ]; then
            ...
            if command -v wasm-objdump &> /dev/null; then
                if wasm-objdump -x pkg/aegis_crypto_core.wasm | grep -q "memory"; then
            ...
            elif command -v wasm-dis &> /dev/null; then
                ...
                print_warning "No WASM analysis tools available - skipping memory verification"
            fi
        fi
...
print_success "🎉 Direct WASM build process completed successfully!"
print_status "📦 Output files are in the pkg/ directory"
print_status "🌐 Ready for browser deployment"
print_status "💡 This build bypassed wasm-pack to ensure our memory configuration is respected"
