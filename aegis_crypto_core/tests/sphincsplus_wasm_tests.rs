...
mod wasm_tests {
    ...
    #[wasm_bindgen_test]
    fn test_sphincsplus_wasm_keygen() {
        let keypair = sphincsplus_keygen().expect("WASM keygen should succeed");
    ...
    #[wasm_bindgen_test]
    fn test_sphincsplus_wasm_sign_verify() {
        let keypair = sphincsplus_keygen().expect("WASM keygen should succeed");
        let public_key = keypair.public_key();
        let secret_key = keypair.secret_key();
        let message = b"WASM test message for SPHINCS+";

        let signature = sphincsplus_sign(&secret_key, message).expect("WASM signing should succeed");
        ...
        let is_invalid = sphincsplus_verify(&public_key, wrong_message, &signature);
        assert!(!is_invalid, "WASM verification should fail with wrong message");
    }

    #[wasm_bindgen_test]
    fn test_sphincsplus_wasm_tampered_signature() {
        let keypair = sphincsplus_keygen().expect("WASM keygen should succeed");
        let public_key = keypair.public_key();
        let secret_key = keypair.secret_key();
        ...
        if !tampered.is_empty() {
            tampered[0] ^= 0xff;
            let is_valid = sphincsplus_verify(&public_key, message, &tampered);
            assert!(!is_valid, "WASM verification should fail for tampered signature");
        }
    }

    #[wasm_bindgen_test]
    fn test_sphincsplus_wasm_invalid_inputs() {
    ...
    #[wasm_bindgen_test]
    fn test_sphincsplus_wasm_keypair_methods() {
        let keypair = sphincsplus_keygen().expect("WASM keygen should succeed");
